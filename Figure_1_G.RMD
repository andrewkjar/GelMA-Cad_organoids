...
```{r}
table(so$orig.ident, so$course_cell_type)
```
...
so.DLEN <- subset(so, course_cell_type%in% c("Radial glia"))
```
...
```
```{r}
subset(gsea_DEG.GelMA_v_Matrigel@result, Description == 'Ribosome')[,11]
```
...
# Sub comparisons
Idents(so.DLEN) <- so.DLEN$orig.ident
DEG.GelMA_soft_v_Matrigel <- FindMarkers(so.DLEN, ident.1 = 'GelMA_LC', ident.2 = 'Matrigel', logfc.threshold = 0)
DEG.GelMA_stiff_v_Matrigel <- FindMarkers(so.DLEN, ident.1 = 'GelMA_HC', ident.2 = 'Matrigel', logfc.threshold = 0)
```
...
```
```{r}
subset(gsea_DEG.GelMACad_v_Matrigel@result, Description == 'Ribosome')[,11]
```
...
# Sub comparisons
Idents(so.DLEN) <- so.DLEN$orig.ident
DEG.GelMACad_soft_v_Matrigel <- FindMarkers(so.DLEN, ident.1 = 'GelMACad_LC', ident.2 = 'Matrigel', logfc.threshold = 0)
DEG.GelMACad_stiff_v_Matrigel <- FindMarkers(so.DLEN, ident.1 = 'GelMACad_HC', ident.2 = 'Matrigel', logfc.threshold = 0)
```
```{r}
# Plotting
...
```
```{r}
subset(gsea_DEG.stiff_v_soft@result, Description == 'Axon guidance')[,11]
...
# Sub comparisons
Idents(so.DLEN) <- so.DLEN$orig.ident
DEG.GelMA_stiff_v_soft <- FindMarkers(so.DLEN, ident.1 = 'GelMA_HC', ident.2 = 'GelMA_LC', logfc.threshold = 0)
DEG.GelMACad_stiff_v_soft <- FindMarkers(so.DLEN, ident.1 = 'GelMACad_HC', ident.2 = 'GelMACad_LC', logfc.threshold = 0)
```
```{r}
# Plotting
...
# Sub comparisons
Idents(so.DLEN) <- so.DLEN$orig.ident
DEG.GelMA_soft_v_GelMACad_soft <- FindMarkers(so.DLEN, ident.1 = 'GelMA_LC', ident.2 = 'GelMACad_LC', logfc.threshold = 0)
DEG.GelMA_stiff_v_GelMACad_stiff <- FindMarkers(so.DLEN, ident.1 = 'GelMA_HC', ident.2 = 'GelMACad_HC', logfc.threshold = 0)
```
```{r}
# Plotting
